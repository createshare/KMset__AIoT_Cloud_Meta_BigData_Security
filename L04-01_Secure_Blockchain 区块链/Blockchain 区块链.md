...
密码学主要分为：编制密码、破译密码。在区块链的应用中，最主要的是研究编制密码。



SHA-256 哈希算法，成为区块链“探矿”的主要算法。
在共识算法方面，出现了如 POW、DPOS、拜占庭容错算法等。

在区块链中，比特币的账户就是通过 ECDSA 进行加密的。

在区块链的资产当中，私钥保护着用户所有的数字资产，用户最好保证私钥处于封闭的环境中，不在网络中流通，防止网络黑客。



# 区块链的数据元素

区块链的数据元素主要由 3 部分组成：哈希结构、密码算法、Merkle 树。

哈希和 Merkle 树共同作用，成为区块链数据存储和索引的关键部分。

Merkle 树功能要点：

- Merkle 树是树的节点结构，由左节点指向左树，右节点指向右树，节点头存储交易信息。
- 每个交易存储的地址是通过 Hash 进行计算，每次索引下一步都需要 Hash 计算。
- 哈希 Key 值，父节点的值 = 左节点 Key + 右节点 Key。
- Merkle 树是区块链存储的主要数据结构，经常配合区块链 SHA-256 算法进行使用。



# 区块链的结构体系

区块链是一项复杂的技术，也是综合的技术方案，它具有一个完整的结构体系。

根据区块链的结构，可以将其分为以下几个层次：

- 数据层：区块链通过规格相同的区块存储和哈希地址计算，形成链式的存储结构。
- 网络层：区块链本质上是P2P的网络结构，每一个节点都进行数据计算和同步，通过节点和节点之间的网络通信共同维护区块链网络。
- 共识层：处理区块链区块数据的共识验证，保证去中心化数据的有效性和同步性。
- 激励层：运用经济因素的技术，通过代币或者Token等发行和获得方式，促进用户加入到区块链，并且提供计算能力，常用在公链当中。
- 合约层：区块链 2.0 发展的技术，它通过脚本代码和算法提供智能合约方，合约运行在区块链的虚拟机上，提供给业务交易的可信凭证。
- 应用层：常用于区块链业务的搭建，包括DApp的开发。区块链的业务提供可以信任的合约形式来处理交易。

![区块链的结构图谱](figures/区块链的结构图谱.jpg)
