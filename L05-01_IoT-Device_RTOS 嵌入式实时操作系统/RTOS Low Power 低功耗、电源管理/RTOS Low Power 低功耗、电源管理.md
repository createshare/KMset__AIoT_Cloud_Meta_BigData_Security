...
## 系统状态: 运行状态 (RUN) 和休眠状态 (Sleep)  

- 运行状态 (RUN)

  - 运行状态控制 CPU的频率，适用于变频场景；  

  - 运行状态通常用于改变 CPU 的运行频率，独立于休眠模式。当前运行状态划分了四个等级：高速、正常、中速、低速。
...
- 休眠状态 (Sleep)  
  ...
  - 休眠状态根据 SOC 特性实现休眠 CPU，以降低功耗。  

  - 休眠状态也就是通常意义上的低功耗状态，通过关闭外设、执行 SOC 电源管理接口，降低系统功耗。  

  - 休眠状态又分为六个模式，呈现为金字塔的形式。随着模式增加，功耗逐级递减的特点。  

  - **注意:** 因各家芯片差异，功耗管理的实现也不尽相同。

    | 模式                   | 级别 | 描述                                                         |
    | ---------------------- | ---- | ------------------------------------------------------------ |
    | PM_SLEEP_MODE_NONE     | 0    | 系统处于活跃状态，未采取任何的降低功耗状态。<br />在此模式下，CPU和所有片上其它资源都正常工作运行。该模式是系统整体功耗的最主要组成部分。 |
    | PM_SLEEP_MODE_IDLE     | 1    | 空闲模式，该模式在系统空闲时停止 CPU 和部分时钟，任意事件或中断均可以唤醒 |
    | PM_SLEEP_MODE_LIGHT    | 2    | 轻度睡眠模式， CPU 停止，多数时钟和外设停止，唤醒后需要进行时间补偿。<br />该模式主要与CPU有关。当CPU进入休眠状态后，其时钟移除。CPU这时对总功耗的唯一影响就是静态功耗，因为这时已经没有时钟开关切换工作，也就不会有动态功耗。ADC和比较器等其它外设在此模式下可用。 |
    | PM_SLEEP_MODE_DEEP     | 3    | 深度睡眠模式， CPU 停止，仅少数低功耗外设工作，可被特殊中断唤醒<br />此电源模式下即便是系统时钟也被禁用，所以在此模式下所有高频资源都不可用。不过，这些资源的当前状态不受影响，也就是说CPU寄存器、 SRAM等的当前状态不受影响。由于高频时钟被禁用，因此能节约开关消耗的功率。通常情况下，深度休眠模式提供低频时钟运行的选项，低频时钟可用来驱动定时器等低频资源。此外，该模式也允许开发人员使用I2C从设备等通信协议块，其无需器件自身生成时钟。然而，模块仍然可通电。该模式对功耗的影响主要在于片上所有时钟的静态功耗。 |
    | PM_SLEEP_MODE_STANDBY  | 4    | 待机模式， CPU 停止，设备上下文丢失 (可保存至特殊外设)，唤醒后通常复位。<br />在此模式下，所有时钟都关闭，包括低速振荡器。片上所有资源，除了用于外部事件触发唤醒的资源以外全都断电。由于本模式下几乎所有组件都断电，因此该模式能减少静态和动态功耗组件，从而实现最低功耗。 |
    | PM_SLEEP_MODE_SHUTDOWN | 5    | 关断模式，比 Standby 模式功耗更低，上下文通常不可恢复，唤醒后复位。<br />所有外设断电，即使是RAM和CPU寄存器的内容也不保持。这种模式下仅保持IO引脚的状态。从这种模式下唤醒会进入芯片重启动。 |
